...
system following the strategies found in traditional modal logic books~\cite{Blackburn-ModalLogics,Popkorn_1994},
which was not present in the earlier version of the paper.
...
the subject of Section~\ref{sec:case}, showcasing an use of our library. Related works are discussed
...
%
All source code in this article has been formalized in Coq version 8.18 and
...
``BHK-in\-ter\-pre\-ta\-tion''\footnote{Abbreviation of Brouwer, Heyting,
  ...
  known as the Curry-Howard correspondence.}, where types represent
...
Every result stated has been fully checked. We note, though, that the
completeness result (Section \ref{sec:completeness}) relies on the excluded
middle as an additional axiom, given that our completeness proof is fully classical;
we also axiomatize that formulas (defined below) are countable, and leave as
future work to properly demonstrate this.
...
\end{minted}
Notation \mintinline{coq}{M ' w ||- p} denotes the valuation of a formula at a given world \verb|w| of the
...
Suppose that \mintinline{coq}{A; Empty |-- p} does not hold.
...
\mintinline{coq}{A; Empty |-- p} is false, by the determination lemma, we know
that \verb|p| is not true on the canonical model \verb|M|. Conclusion then follows directly
by our hypotheses, \mintinline{coq}{Empty ||= p} and \mintinline{coq}{(A; Empty |-- p) -> False} (which includes showing that all formulas in the empty set are valid).
...
%
The purpose of this exercise is to formally prove that the existence of fixed
points (defined below) in a modal logic system implies the GL axiom: for any formula $\Psi$, it
holds that $\Box(\Box\Psi \to \Psi) \to \Box\Psi$. Among the systems defined in
this paper, System GL possesses the fixed point property, though this proof is
non-trivial \cite{fixpoint}; but, in this case, our conclusion becomes rather
uninteresting: the GL axiom is already an axiom in System GL, which trivializes
the result. Still, we justify this case study as other systems that do not have
GL as an axiom but do possess the fixed point property (see \cite{fixpoint}) are
definable, and the main purpose for this code is to formalize a piece of modal
logic proof within Coq by using our framework.
...
which is parameterized by a system of modal logic, \mintinline{coq}{A}, and a
...
\noindent We can now write down L\"ob's theorem as follows (representing the axiom through derivations):
