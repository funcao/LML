...
K and it includes the axiom $\Box(A \to B) \to (\Box A \to \Box B)$
...
logics, one for the $K$ axiom and cases for using hypothesis, the rules modus
...
the literature is not always clear about the difference of those.

{\color{red} TORRENS: Escrever definições de completude}
...
representation of deduction we are able to prove soundness and completeness theorems, and, in order to demonstrate the applicability of the proposed work, prove L\"ob's theorem
...
{\color{red} TORRENS: Dedução natural e táticas.}
