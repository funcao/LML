...
\begin{itemize}
  ...
  \item We prove soundness and completeness for the Hilbert deductive system. For the
...
This article is an extended version of our SBLP 2022 work~\cite{Silveira22}. In this paper we include
a detailed discussion about our formalization of a completeness proof for the Hilbert deductive
...
\subsection{An overview of Coq proof assistant.}
...
between \mintinline{coq}{formula} and Coq's \mintinline{coq}{Prop}s.
...
Hilbert proof system with respect to the Kripke semantics. The proof consists of two
basic steps: 1) the proof of the existence of maximal consistent sets and; 2) the
construction of a cannonical model from the maximal consistent set. We describe these
steps separately.

\paragraph{Maximal consistent sets} The first step on the completeness proof is the construction of
a \emph{maximal consistent sets}, $\Delta$, of formulas for $\Gamma$ and $\varphi$.  The $\Delta$ set
...
Which states that it is not possible to deduce \verb|p| and \verb|~ p|, for any formula \verb|p|.
...
which satisfies the following property: $\forall \varphi . \texttt{decode(encode }\varphi\texttt{)} = \varphi$.
...
| Insert_skip: forall D, Subset D (Insert None D)
| Insert_valid:
  forall D p, Consistent A (Extend p D) -> Subset (Extend p D) (Insert (Some p) D)
| Insert_invalid:
  forall D p, ~ Consistent A (Extend p D) -> Subset (Extend [! ~p !] D) (Insert (Some p) D).
\end{minted}
...
In this case, we add the negation of \verb|p| to the theory.
...
\begin{minted}{coq}
Theorem lindebaum : Consistent A G -> exists D, Consistent A D /\ Maximal D /\ Subset G D.
\end{minted}
This concludes our formalization of the existence of the maximal consistent sets for modal logics formula.
...
$\mathcal{M} = \langle\mathcal{F},\mathcal{V}\rangle$ is such that:
\begin{itemize}
  \item The frame $mathcal{F} = \langle W, \mathcal{R}\rangle$ is
        \begin{itemize}
          \item $\mathcal{W}$ is the set of all maximal consistent sets of formulas;
          \item $\mathcal{R}(w_{1},w_{2})$ iff $\Box \varphi \in w_{1}$ implies $\varphi \in w_{2}$;
        \end{itemize}
  \item The labelling function $\mathcal{V}$ is such that $\mathcal{V}(A) \in \mathcal{W}$ and $p\in \mathcal{V}(A)$,
        for all propositional variables $A$.
\end{itemize}

The canonical model is formalized in Coq as follows. First, we define the type \mintinline{coq}{W} that pack
...
\end{minted}
The proof is divided in two more lemmas for each implication direction using the truth lemma. Finally, the completeness theorem is stated as follows:
\begin{minted}{coq}
...
If the theory \verb|G| is inconsistent, the desired result follows.
So, assume that \verb|G| is consistent. Since \verb|G| is consistent, it
...
including support for multimodalities (fusion of modal logics) and
