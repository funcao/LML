...
The use of these ``weaker'' modal logics can avoid some undesirable properties 
brought by the normality. In epistemic logics, for instance, normal systems 
have the problem of logical omniscience~\cite{Hocutt1972}: due to the necessity 
rule, taking the modality as a knowledge operator, all theorems must be 
known (in practice, this is not true).
...
but we believe that using mature tools, like a proof assistant, are a better option for
...
works, let us review its type (below, remeber that \mintinline{coq}{A : axiom -> Prop}
...
Ax: forall (t: theory)
           ...
           A a -> instantiate a = f -> deduction A t f
\end{minted}
Notice that the first parameter of constructor \mintinline{coq}{Ax} is a value
...
While we have completely proved the soundness theorem, the current version of
our library does not have a completeness theorem. 
...
We represent the modal fixed points by definition \mintinline{coq}{fixed_point},
which is parameterized by a system for modal logics, \mintinline{coq}{S}, and a
theory, \mintinline{coq}{g}, and encodes the logical equivalence between a
formula \mintinline{coq}{p} and its fixpoint \mintinline{coq}{f ([] p)}. 
\begin{minted}{coq}
...
Using the definition of fixed points, we can state L\"ob's theorem as follows:
\begin{minted}{coq}
Theorem Lob:
  forall P, fixed_point K4 nil ->
  (K4; nil |-- [! []P -> P !]) ->
  (K4; nil |-- [! P !]).
\end{minted}

The proof starts by assuming an arbitrary formula $P$, the existence of modal fixed
...
Next, from the existence of modal fixed points for every formula,
...
\end{minted}
Now, we are able to deduce that $\Psi \to (\Box \Psi\to P)$ from the previous
...
apply modal_ax5 in H2; try repeat constructor ;
   try eassumption.
\end{minted}
The \mintinline{coq}{assert} tactic allows the definition of
...
Now, using the axiom \textbf{K} we can deduce $\vdash \Box \Psi \to
...
The next proof step involves showing that
...
Using $\vdash \Box (\Box \Psi \to P)\to (\Box \Box \Psi \to \Box P)$ and
$\vdash \Box \Psi \to\Box (\Box \Psi \to P)$ we can deduce
$\vdash \Box \Psi \to \Box \Box \Psi \to \Box P$ using composition of
the implication.
\begin{minted}{coq}
...
Now, we can show that $\vdash\Box\Psi\to\Box\Box\Psi$ using
...
From $\vdash \Box\Psi \to \Box P$ and $\vdash \Box P \to P$, we can
...
Next, we can conclude $\vdash \Box (\Psi \to P) \to \Psi$ from
...
From $\vdash (\Box\Psi \to P) \to \Psi$ and $\vdash \Box \Psi \to P$
...
By necessitation rule, we can deduce $\vdash \Box \Psi$
\begin{minted}{coq}
assert (K4; nil |-- [! []psi !]) as H13.
apply Nec ; try eassumption.
\end{minted}

Finally, the desired conclusion $\vdash P$ follows from
...
This concludes the formalization of the L\"ob's theorem using
...
All developments considered systems K, T and K4 and the unique formalization that
includes system GL is the library described in this work.
...
Authors stabilished a semantic type soundness proof by defining a Kripke-style 
